...
test.describe('Photos and Events', () => {
  ...
  test('photography page has link to events', async ({ page }) => {
    ...
    await expect(page.locator('a[href="/events"]')).toContainText('All events')
  })
  ...
  test('events page has link to photography', async ({ page }) => {
    ...
    await expect(page.locator('a[href="/photography"]').last()).toBeVisible()
    await expect(page.locator('a[href="/photography"]').last()).toContainText('Portfolio')
  })
  ...
  test('events display with correct information', async ({ page }) => {
    ...
    if (eventCount > 0) {
      ...
      const dateAndCountText = await firstEvent.locator('p.sub').textContent()
  ...
  test('PhotoSwipe lightbox functionality', async ({ page }) => {
    ...
    if (imageCount > 0) {
      ...
      await expect(page.locator('.pswp__button--arrow--next')).toBeAttached()
      ...
      const lightboxClasses = await page.locator('.pswp').getAttribute('class')
  ...
  test('events are sorted by date', async ({ page }) => {
    ...
    if (eventCount > 1) {
      // Get all event dates
      const eventDates = await eventSections.locator('p.sub').allTextContents()
      
      // Extract dates from the text (format: "September 2025 · X photos")
      const dates = eventDates.map(text => {
